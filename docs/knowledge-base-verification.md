...
### 11.12 动态健康度与真实标准文档复测（2026-07-31）

本轮针对健康度只统计标准、软件清单写死的问题，部署并验证提交 `5ad02bd`、
`3b3966d` 与 `446aacb`：

- 标准覆盖矩阵的软件清单改为读取后台管理中启用的软件类型；已发布标准中暂未进入
  后台清单的软件仍并入分母，避免历史内容消失；目录调用失败时显式返回
  `catalogStatusReliable=false`，不再静默给出错误分母。
- 健康度把「标准覆盖」与「知识内容组成」分开。新增上传文档、标准文档、其他来源、
  经验页面（生效/草稿）和知识内容总数；经验页面只计入内容，不填充参数/部署/监控/
  应急四类标准格子。
- 前端健康度卡片统一读取 `/api/knowledge/corpus-health`，移除旧 stats 口径和不可靠的
  `indexedChunks` 展示；覆盖区明确标为「标准覆盖度」并显示后台软件数量。
- 首次远端门禁发现 `ai-service` 缺少 Spring Cloud LoadBalancer 依赖，请求误发到
  `core-service:80`。TC-AI-005 先复现负载均衡客户端为空，再补依赖；修复后目录调用
  命中 Nacos 中注册的 8084 端口，`catalogStatusReliable=true`。

远端 `/app/infra-portal` 在部署前备份到
`/app/infra-portal/backups/pre-3b3966d-health-20260731`。`core-service` 与前端镜像为
`3b3966d-health`，`ai-service` 修复镜像为 `3b3966d-health-lb1`，三个容器均为
healthy。完整后端 26 模块共 290 个测试通过；前端 63 个测试与生产构建通过。

#### 11.12.1 动态分类、经验页面和内容组成

运行时使用唯一 runId `D7380CBC`，所有断言完成后删除本轮来源、向量、页面和软件类型：

| 阶段 | 软件/矩阵 | 知识内容组成 | 断言 |
|---|---|---|---|
| 基线 | 20 类，`0/80`，缺口 80 | 来源 6、上传 6、经验 0，总内容 6 | 目录状态可靠 |
| 新增后台软件类型 | 21 类，`0/84`，缺口 84 | 不变 | 新类型自动增加参数/部署/监控/应急 4 格 |
| 新增经验草稿 | 矩阵仍为 `0/84` | 经验 1、草稿 1，总内容 +1 | 经验内容不冒充标准覆盖 |
| 上传 9 份文档 | 矩阵仍为 `0/84` | 来源 15、上传 15、经验 1，总内容 16 | 上传文档单独计数 |
| 清理后 | 恢复 20 类、`0/80`、缺口 80 | 恢复来源 6、上传 6、经验 0、总内容 6 | 与基线逐字段相等，无清理错误 |

#### 11.12.2 Downloads 真实文档解析与检索

测试数据来自 `Downloads.zip` + `Downloads.z01` 中的 `中间件技术标准.zip`。外层 CSV
不参与测试，临时解压副本已删除。内层共 41 个有效 DOCX/XLSX 文件、展开约 39 MB，
本轮选取 9 份覆盖部署、应急、监控和版本基线；全部通过项目自身
`POST /api/knowledge/upload` 的 Tika 解析、切片、bge-m3 向量化和 Milvus 入库链路，
未使用外部解析器生成测试结论。

| 文档 | 格式 | 切片数 | 自检索排名 |
|---|---:|---:|---:|
| Tomcat 部署标准 | DOCX | 48 | 1 |
| Nginx 部署标准 | DOCX | 70 | 1 |
| WebLogic 部署标准（5.7 MB 长文档） | DOCX | 92 | 1 |
| RocketMQ 部署标准 | DOCX | 72 | 1 |
| Kafka 应急手册 | DOCX | 8 | 1 |
| RabbitMQ 应急手册 | DOCX | 8 | 1 |
| Redis 监控模板 | XLSX | 4 | 1 |
| Nacos 监控模板 | XLSX | 4 | 1 |
| 中间件版本基线和生命周期 2026Q1 | XLSX | 8 | 1 |

合计 314 个切片，项目预览接口返回的切片数与上传结果逐份一致。每份文档从预览切片
中取 64~120 字作为查询，只在报告中保存长度与 SHA-256，不保存标准正文；9/9 均在
Top-1 命中原文档。该结果证明真实 DOCX/XLSX 的解析、长文切片、表格切片、向量化和
检索闭环可工作，但它是自检索冒烟测试，不替代独立业务问法的 Recall@5 黄金集评估。

真实浏览器复核显示「知识内容 6、经验沉淀 0、上传文档 6、标准文档 0」，标准覆盖区
显示「后台软件 20 类、0/80 格」；页面无重复分隔符、无元素遮挡，控制台无 WARN / ERROR。

#### 11.12.3 当前结论

健康度统计口径和工具链已达到可用标准，KBV-015、KBV-017（前端误导展示）、
KBV-019、KBV-020 已解决；新发现的 AI 目录负载均衡缺陷也已修复并补回归。

**当前知识库内容仍未达到生产可用的完整性标准。** 清理测试夹具后的真实快照只有
6 份上传文档、0 条经验页面、0 份受管标准，后台 20 类软件的标准覆盖为 `0/80`。
因此现在最大的瓶颈确实是正式数据缺失，而不是解析或基础检索链路。KBV-016 的测试
语料清理脚本边界和 KBV-018 的跨分类 `relink` 权限问题仍需单独处理。

### 11.13 生产检索质量待解决问题（2026-07-31）

11.12 的 9/9 Top-1 结果是从项目预览切片中抽取原文做自检索，证明了解析、切片、
向量化和检索闭环，但不能证明用户换一种说法、只描述现象、遇到冲突标准或库内存在
错误文档时仍能得到正确答案。生产可用评估新增以下问题：

| 编号 | 严重度 | 当前证据与风险 | 建议处理 | 验收标准 | 状态 |
|---|---|---|---|---|---|
| KBV-021 | 高 | 当前 `KnowledgeService.search` 对原问题执行一次 dense + BM25 混合检索，没有软件别名和运维同义词归一化；`WLS/WebLogic`、`连接耗尽/连接池打满` 等改写可能漏召回 | 建立后台可维护的软件别名与运维术语表；先做确定性归一化，再让原问题和扩展问题并行检索并用 RRF 合并去重 | 独立同义问法集 `Recall@5 >= 85%` | 待实现 |
| KBV-022 | 高 | 模糊故障描述通常同时缺软件名、错误码和处理目标，单次相似度检索容易只命中现象相似但不可执行的内容 | 提取软件、现象、错误码、指标和操作阶段；拆分为症状、原因、处理、验证子查询；明确元数据时增加软件和标准类型过滤 | 独立模糊故障集 `Recall@5 >= 80%`，且 Top-5 至少包含一份可执行处理依据 | 待实现 |
| KBV-023 | 高 | 参数精确查询已经绕过 RAG，健康度也能发现同参数不同值；但一般检索/生成链路尚未保证遇到跨版本、跨来源冲突时强制展示全部证据，存在静默挑选一个值的风险 | 向量元数据补充版本、状态、生效时间和权威级别；入库时登记冲突；回答门控要求列出冲突值、版本和来源，禁止无提示合并 | 参数答案准确率 100%，已构造冲突漏报率 0，回答包含可追溯来源 | 待实现 |
| KBV-024 | 高 | 现有过滤器支持分类、软件和来源类型，但默认不能排除未发布、过期、低权威或软件相近的错误文档，RRF 后也没有面向问题相关性的二次重排 | 默认过滤草稿和失效来源；按软件、场景、版本和权威级别重排；增加最低相关度与证据门槛，证据不足时明确拒答 | Top-5 错误文档率 `<= 10%`，无证据问题不得生成确定性结论 | 待实现 |

这四项共享同一个前置条件：人工黄金集必须为每份代表性标准至少编写标准问法、同义
问法、模糊现象问法、冲突问法，并配置期望来源和禁止命中的干扰来源。实现顺序建议为：

1. 先补版本、状态、生效时间、权威级别和标准类型元数据，并建立人工黄金集；
2. 再实现术语归一化、故障查询拆分和多查询召回；
3. 最后增加 reranker、证据门控和冲突回答策略，用同一黄金集做前后对比。

在 KBV-021~024 达标前，知识库可用于文档检索试运行，但不能宣称复杂故障问答和参数
结论已经达到生产质量。
