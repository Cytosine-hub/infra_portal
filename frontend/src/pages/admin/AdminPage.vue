...
const props = defineProps({
  ...
  canManageForumTags: { type: Boolean, default: false }
})
...
const allSections = [
  ...
  { key: 'forumTags', label: '论坛管理', sysAdmin: false, forumAdmin: true },
  { key: 'users', label: '用户管理', sysAdmin: true },
  { key: 'settings', label: '系统设置', sysAdmin: true }
]

const visibleSections = computed(() => allSections.filter((section) =>
  (!section.sysAdmin || props.isSysAdmin)
  && (!section.forumAdmin || props.canManageForumTags)
))
</script>
