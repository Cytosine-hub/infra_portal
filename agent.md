...
│   ├── src/shared/          # 跨岗位复用：shared/api/createModuleApi.js、shared/jobs/（JobModuleEntry/JobNavigation/JobWorkspace/createJobModule/jobFilter/useJobFilter）
│   ├── src/components/      # 业务组件（DocumentEditor、Forum*、KnowledgePanel/KnowledgeGraph、DiagnosticsPanel、WikiPanel、previews/…）
│   ├── src/components/ui/   # Base* 通用组件（BaseButton/BaseInput/BaseModal/BaseMultiSelect/ConfirmDialog/DataTable/EmptyState/FormModal/LoadingSpinner/PageHeader/StatusBadge/TabNav/Toast/Toolbar，仅用设计令牌）
...
│   └── test/                # Vitest 测试（portal-structure.test.js、portal-layout-optimization.test.js）
├── docs/                # development-standards.md、code-review-rules.md、microservices-stage1~6、startup-manual.md、milvus/zabbix/wiki 专项文档等
├── scripts/             # deploy.sh、package-for-deploy.sh、start-local-*.ps1、run-local-*.ps1、services.env.example、systemd *.service
├── db/ deploy/ release/ # DDL（init.sql、seed.sql、schema、升级脚本）、Milvus 离线部署物、发布说明
├── commands/ examples/  # 中间件命令库（middleware-types.json）、Zabbix 示例
└── AGENTS.md PORTAL_PILOT.md README.md RELEASE_v0.2.0.md  # 引导、试点说明、总览与发布记录
```
