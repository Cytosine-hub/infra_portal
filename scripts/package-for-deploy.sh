...
echo "  后端 JAR: target/middleware-resource-manager-0.0.1-SNAPSHOT-exec.jar"
...
# 后端
cp "$PROJECT_DIR/target/middleware-resource-manager-0.0.1-SNAPSHOT-exec.jar" "$OUTPUT_DIR/backend/"

# 前端
cp -r "$PROJECT_DIR/frontend/dist/"* "$OUTPUT_DIR/frontend/dist/"

# DDL
cp "$PROJECT_DIR/src/main/resources/db/knowledge_ddl.sql" "$OUTPUT_DIR/db/"
...
# 默认配置（供参考）
cp "$PROJECT_DIR/src/main/resources/application.yml" "$OUTPUT_DIR/backend/application.yml.example"
